...
**Branch model:** the appset's SCM generator discovers this repo by reading
`argo/envs/dev/config.json` on the default branch (`main`), where it carries an extra
`"targetRevision": "dev"`. The actual manifests under `argo/overlays/dev` are synced
from the `dev` branch, where `config.json` omits `targetRevision`.
...
This repo is **single-service**.

## Services (dev)

| Service | Host (kept from ECS) | Health |
| ------- | -------------------- | ------ |
| `quiz0` | quiz0-dev-topia.topia-rtsdk.com | `/` |

In dev the direct Ingress template is dropped and the app is fronted through the KEDA
interceptor on the same host, so a scaled-to-zero Deployment is woken by the first request.
...
| `targetRevision` | (`main` copy only) branch the appset syncs manifests from (`dev`). |
